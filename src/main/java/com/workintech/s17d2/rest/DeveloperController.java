...
public class DeveloperController {
    ...
    public void init(){
        ...
        this.developers.put(1,new JuniorDeveloper(1,"Burak",1000d));
    }
    @PostMapping
...
public DeveloperResponse getById(@PathVariable("id") int id){
        ...
        if(foundDeveloper==null){
            return new DeveloperResponse(null,id+" ile kayıt bulunamadı",HttpStatus.NOT_FOUND.value());
        }
        return new DeveloperResponse(foundDeveloper,"search başarılı",HttpStatus.OK.value());
    }
    @PutMapping("/{id}")
    public DeveloperResponse update(@PathVariable("id")int id,@RequestBody Developer developer){
        ...
        return new DeveloperResponse(newDeveloper,"Güncelleme başarılı",HttpStatus.OK.value());
    }
    @DeleteMapping("/{id}")
